  ...
  return (
    <div className="col-span-12 space-y-8">
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col">
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-white rounded-t-2xl gap-3">
          ...
          <div className="flex items-center gap-2">
            {/* Online/offline indicator for selected device */}
            {targetDevice && (
              <span
                  ...
                  <tr key={game.id} className="hover:bg-slate-50/70 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-right">
                       <div className="flex items-center justify-end gap-1.5 transition-all">
                         ...
                         <button 
                           onClick={() => handleRemoteRestore(game)}
                           disabled={!targetDevice}
                           className={`p-2 rounded-lg border transition-all ${
                             targetDevice
                               ? 'text-emerald-600 hover:bg-white border-transparent hover:border-slate-200'
                               : 'text-slate-300 cursor-not-allowed border-transparent'
                           }`}
                           title={targetDevice ? "Khôi phục về app đã cài trên máy" : "Chọn thiết bị trước khi khôi phục"}
                         >
                           <RotateCcw className="w-4 h-4" />
                         </button>
                         {restoreStatus && (restoreStatus.status === 'Pending' || restoreStatus.status === 'Running') && (
                           <button
                             onClick={() => handleCancelRestore(restoreStatus.id)}
                         ...
                         {restoreStatus && ['Failed', 'Timeout', 'Cancelled'].includes(restoreStatus.status) && (
                           <button
                             onClick={() => handleRetryRestore(restoreStatus.id)}
                             className="p-2 text-amber-600 hover:bg-white rounded-lg border border-transparent hover:border-slate-200 transition-all font-bold"
                             title="Retry khôi phục"
                           >
                             R
                           </button>
                         )}
