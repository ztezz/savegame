...
export default function Auth({ onLogin }: { onLogin: (token: string, user: any) => void }) {
  ...
  const canSubmit = trimmedUsername.length > 0 && password.trim().length > 0 && !loading;
        ...
        >
          <div className="flex items-center gap-4 mb-12">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tight">
                CloudSave<span className="text-indigo-300">Hub</span>
              </h1>
              <p className="text-[11px] text-slate-400 uppercase tracking-widest font-bold">
                Đồng bộ dữ liệu game
              </p>
            </div>
          </div>

          <h2 className="text-5xl font-black leading-tight mb-6">
            Quản lý bản lưu ổn định cho mọi thiết bị.
          </h2>
          ...
          <div className="grid grid-cols-2 gap-4 mt-12">
            ...
            <div className="bg-white/6 border border-white/10 rounded-lg p-5">
              ...
              <div className="text-sm font-bold mb-1">Kiểm soát an toàn</div>
              <p className="text-xs text-slate-400 leading-5">Xác thực tài khoản trước khi truy cập dashboard.</p>
            </div>
          </div>
        </motion.div>
      </section>
        ...
        >
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-slate-500 leading-6">
              {isLogin
                ? 'Xác thực để truy cập dashboard CloudSaveHub.'
                : 'Tạo tài khoản để bắt đầu quản lý dữ liệu đồng bộ.'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            <AnimatePresence>
              {error && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -8 }}
                  className="p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm font-semibold flex items-start gap-2"
                >
            ...
            <div className="space-y-2">
              <label htmlFor="username" className="text-xs uppercase tracking-widest font-black text-slate-500 ml-1">
                Tên đăng nhập
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-12 pr-4 py-4 rounded-lg border bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition placeholder:text-slate-300 font-medium ${
                    error && !trimmedUsername ? 'border-red-300 bg-red-50' : 'border-slate-200'
                  }`}
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label htmlFor="password" className="text-xs uppercase tracking-widest font-black text-slate-500 ml-1">
                Mật khẩu
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-12 pr-12 py-4 rounded-lg border bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition placeholder:text-slate-300 font-medium ${
              ...
              {!isLogin && (
                <p className="text-xs text-slate-500 ml-1">Mật khẩu cần tối thiểu 6 ký tự.</p>
              )}
            </div>
            ...
            <button
              type="submit"
              disabled={!canSubmit}
              className="w-full py-4 bg-slate-950 hover:bg-slate-800 text-white rounded-lg font-bold transition shadow-lg shadow-slate-200 flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
