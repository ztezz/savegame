    ...
    <aside className="hidden lg:flex w-64 bg-slate-900 text-white flex-col border-r border-slate-800 shrink-0">
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        ...
        <button 
          ...
          className={`w-full flex items-center gap-3 p-3 rounded-xl text-sm font-semibold transition-all ${activeTab === 'drive' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/20' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
        >
        ...
        {(currentUser?.role?.toLowerCase() === 'admin' || currentUser?.username === 'admin') && (
          <button 
            ...
            className={`w-full flex items-center gap-3 p-3 rounded-xl text-sm font-semibold transition-all ${activeTab === 'logs' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/20' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
          >
