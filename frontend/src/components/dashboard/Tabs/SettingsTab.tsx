  ...
  return <div className="col-span-12 space-y-6 px-1 sm:px-0">
    ...
    {isAdmin && <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h3 className="flex items-center gap-2 text-sm font-black uppercase tracking-widest text-slate-800"><FileText className="h-4 w-4 text-indigo-600" />Nhật ký hệ thống</h3>
          <p className="mt-1 text-xs text-slate-500">Các hoạt động gần đây đã được diễn giải để dễ theo dõi hơn.</p>
        </div>
        <span className="rounded-full bg-slate-100 px-3 py-1 text-[11px] font-black text-slate-500">{auditLogs.length} mục mới nhất</span>
      </div>
      <div className="max-h-96 space-y-3 overflow-auto pr-1">
        {auditLogs.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-200 p-6 text-center text-sm text-slate-500">Chưa có nhật ký hệ thống.</div> : auditLogs.map((log: any) => {
