...
const OverviewTab: React.FC<OverviewTabProps> = ({
  ...
  return (
    <div className="col-span-12 space-y-5 sm:space-y-6">
      <section className="relative overflow-hidden rounded-3xl bg-slate-950 px-5 py-6 text-white shadow-xl sm:px-7 sm:py-7">
        <div className="absolute -right-20 -top-24 h-64 w-64 rounded-full bg-indigo-500/20 blur-3xl" />
        <div className="absolute bottom-0 right-1/3 h-28 w-28 rounded-full bg-sky-400/10 blur-2xl" />
        <div className="relative grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center">
          ...
          <div className="flex flex-wrap gap-2 lg:justify-end">
            <button onClick={handleOpenNew} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-xs font-black text-slate-950 transition hover:bg-indigo-50">
              <Upload className="h-4 w-4" /> Tải bản lưu
            </button>
            <button onClick={() => setActiveTab('devices')} className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2.5 text-xs font-black text-white ring-1 ring-white/10 transition hover:bg-white/15">
          ...
          {games.length === 0 ? (
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {games.slice(0, 5).map((game) => (
                <article key={game.id} className="flex items-center gap-3 px-4 py-3.5 transition hover:bg-slate-50 sm:px-5 dark:hover:bg-slate-800/60">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-300"><Gamepad2 className="h-5 w-5" /></div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="mt-5 rounded-2xl bg-slate-950 p-4 text-white">
            <div className="flex items-center gap-3">
              <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${isSyncing ? 'bg-indigo-500' : 'bg-white/10'}`}>
            ...
            {uploadProgress !== null && (
              <div className="mt-4">
                ...
                <div className="h-1.5 overflow-hidden rounded-full bg-white/10" role="progressbar" aria-valuenow={uploadProgress} aria-valuemin={0} aria-valuemax={100}><div className="h-full rounded-full bg-indigo-400 transition-all" style={{ width: `${uploadProgress}%` }} /></div>
              </div>
            )}
          </div>
          ...
          {devices.length === 0 ? (
            ...
            <div className="space-y-2">
              {devices.slice(0, 4).map((device) => {
                ...
                return <div key={device} className="flex items-center gap-3 rounded-xl bg-slate-50 px-3.5 py-3 dark:bg-slate-800/70">
                  <div className={`flex h-9 w-9 items-center justify-center rounded-lg ${online ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-slate-200 text-slate-500 dark:bg-slate-700'}`}>{online ? <Wifi className="h-4 w-4" /> : <WifiOff className="h-4 w-4" />}</div>
          ...
          {syncLogs.length === 0 ? (
            ...
            <div className="space-y-3">
              {syncLogs.slice(0, 5).map((log, index) => {
                ...
                return <div key={log.id || index} className="flex gap-3">
