...
export const authenticateToken = async (req: any, res: any, next: any) => {
  ...
  jwt.verify(token, JWT_SECRET, async (err: any, user: any) => {
    ...
    if (isUsingDatabase()) {
      try {
        const { rows } = await pool.query(
          "SELECT id, username, display_name, avatar_url, role, status FROM users WHERE username = $1 AND status = 'Active'",
          [user.username]
        );
        if (rows.length > 0) {
          req.user = { id: rows[0].id, username: rows[0].username, display_name: rows[0].display_name, avatar_url: rows[0].avatar_url, role: rows[0].role };
